...
/**
 * Move Contract Dialog
 *
 * Modal for moving a contract to a different folder.
 * Shows a recursive folder tree with expand/collapse and highlights
 * the current folder. Cloned from email templates move-template-dialog.
 *
 * SOURCE OF TRUTH KEYWORDS: MoveContractDialog
 */
...
/**
 * Folder icon SVG — matches the folder card design
 */
...
function FolderRow({
  ...
  return (
    ...
    >
      ...
      {/* Contract count */}
      {!isCurrent && folder._count.contracts > 0 && (
        <span className="text-xs text-muted-foreground ml-2">
          {folder._count.contracts}
        </span>
      )}
    </div>
  )
...
function FolderNode({
   ...
   */
  const { data: children, isLoading } = trpc.contracts.listFolders.useQuery(
    ...
    <>
      ...
      {isExpanded &&
        children?.map((child) => (
          <FolderNode
            key={child.id}
            folder={child as FolderTreeItem}
            depth={depth + 1}
...
export function MoveContractDialog({
   ...
   */
  const { data: rootFolders, isLoading } = trpc.contracts.listFolders.useQuery(
    ...
    <Dialog open={open} onOpenChange={(o) => !o && handleClose()}>
      <DialogContent className="sm:max-w-[420px] p-0 gap-0">
        ...
        <ScrollArea className="h-[320px] px-2">
          <div className="space-y-0.5 pb-2">
            ...
            >
              ...
              <span className="text-sm flex-1">All Contracts (Root)</span>
              {isRootCurrent && (
            ...
            {!isLoading &&
              rootFolders?.map((folder) => (
                <FolderNode
                  key={folder.id}
                  folder={folder as FolderTreeItem}
                  depth={0}
                  selectedId={selectedId}
                  currentFolderId={currentFolderId}
                  expandedIds={expandedIds}
        ...
        <div className="flex justify-end gap-2 px-5 py-4 border-t">
          ...
          <Button onClick={() => onMove(selectedId)} disabled={!canMove || isMoving}>
            {isMoving ? (
              <>
