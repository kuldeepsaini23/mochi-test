...
/**
 * EDIT AUTOMATION FOLDER DIALOG COMPONENT
 *
 * Modal for editing automation folder properties (name and color).
 *
 * FEATURES:
 * - Edit folder name
 ...
 *
 * SOURCE OF TRUTH KEYWORDS: EditAutomationFolderDialog
 */
...
interface EditAutomationFolderDialogProps {
...
/**
 * Curated color palette for folders.
 * Based on modern design systems (similar to Google Drive, macOS).
 */
const PRESET_COLORS = [
...
function FolderPreview({ color, name }: { color: string; name: string }) {
  return (
    <div className="flex flex-col items-center gap-2 p-4 rounded-lg bg-muted/30">
      <FolderShape color={color} size={64} />
...
export function EditAutomationFolderDialog({
  open,
  onClose,
  folderId,
  folderName,
  ...
  onSave,
  isSaving = false,
}: EditAutomationFolderDialogProps) {
  // Form state
  const [name, setName] = useState(folderName)
  const [color, setColor] = useState(folderColor || '#3f3f46')

  // Reset form when dialog opens with new folder
  useEffect(() => {
    if (open) {
      setName(folderName)
  ...
  const handleSave = () => {
    ...
    onSave({
      name: trimmedName,
      // null for default color (removes custom color)
        ...
        <div className="space-y-6 py-4">
          ...
          <div className="space-y-2">
                ...
                >
                  {color === preset.value && (
                    <Check
                      className={cn(
                        'w-4 h-4 mx-auto',
                        // Use white check for dark colors, black for light
                        [
                          '#f59e0b',
                          '#eab308',
                          '#84cc16',
            ...
            <div className="flex items-center gap-2 mt-3">
              <Label htmlFor="custom-color" className="text-xs text-muted-foreground whitespace-nowrap">
                Custom:
              </Label>
              <input
                type="color"
