/**
 * Chart01 - Recurring Revenue (MRR/ARR)
 *
 * WHY: Displays monthly/annual recurring revenue trends
 * HOW: Fetches data from dashboard.getRecurringRevenue, renders as stacked bar chart
 *
 * DATA:
 * - actual: Actual MRR/ARR from RECURRING billing type transactions
 * - projected: Calculated growth projection
 *
 * SOURCE OF TRUTH KEYWORDS: RecurringRevenueChart, MRRChart, ARRChart
 */
...
export function Chart01() {
  ...
  // Fetch data
  const { data, isLoading, error } = trpc.dashboard.getRecurringRevenue.useQuery(
    {
  ...
  const mrrChange = totals.mrrChange >= 0 ? `+${totals.mrrChange}%` : `${totals.mrrChange}%`
  const arrTotal = formatCurrency(totals.arr, currency)
  const arrChange = totals.arrChange >= 0 ? `+${totals.arrChange}%` : `${totals.arrChange}%`
  ...
  const isPositive = selectedValue === 'on' ? totals.arrChange >= 0 : totals.mrrChange >= 0
