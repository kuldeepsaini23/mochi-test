 ...
 *
 * Modal for editing folder properties (name and color).
 * Uses the same beautiful styling as the Move To dialog.
 *
 * FEATURES:
 * - Edit folder name
...
export function EditFolderDialog({
  ...
  const handleSave = () => {
    ...
    onSave({
      name: trimmedName,
      color: color === '#3f3f46' ? null : color, // null for default color
    })
  }

  // Check if form has changes
  const hasChanges = name.trim() !== folderName || (color || '#3f3f46') !== (folderColor || '#3f3f46')
                  ...
                  {color === preset.value && (
                    <Check
                      className={cn(
                        ...
                        ['#f59e0b', '#eab308', '#84cc16', '#22c55e', '#14b8a6', '#06b6d4'].includes(preset.value)
                          ? 'text-black'
                          : 'text-white'
                      )}
                    />
