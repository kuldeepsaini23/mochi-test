/**
 * ============================================================================
 * TRIAL STARTED TRIGGER CONFIG
 * ============================================================================
 *
 * Configuration form for the "Trial Started" trigger.
 * Fires when a free trial subscription begins for a product/price.
 *
 * Primary filter: price ID (most specific — identifies exactly which price started a trial).
...
import type { TrialStartedTriggerConfig as TrialStartedConfig } from '../../../_lib/types'
...
interface TriggerTrialStartedConfigProps {
  config: TrialStartedConfig
  onChange: (config: TrialStartedConfig) => void
...
export function TriggerTrialStartedConfig({
  config,
  onChange,
  errors,
}: TriggerTrialStartedConfigProps) {
  const { organizationId } = useAutomationBuilder()
  const { products, isLoading } = useAutomationBuilderData(organizationId)

  /**
   * Filter to only show products that have at least one price with a trial period.
   * A "Digital Marketing Mastery Course" with no trial prices has no business
   * showing up in a "Trial Started" trigger config.
   */
  const trialProducts = useMemo(() => {
    return products
      .map((p) => ({
        ...p,
        prices: p.prices.filter((price) => price.trialDays != null && price.trialDays > 0),
      }))
  ...
  const selectedProduct = useMemo(() => {
    return trialProducts.find((p) => p.id === config.productId)
  }, [config.productId, trialProducts])
  ...
  const handleProductChange = (productId: string) => {
    if (productId === 'any') {
      ...
      const product = trialProducts.find((p) => p.id === productId)
      onChange({
        ...config,
        productId,
   ...
   * Price ID is the most specific filter — it identifies exactly which price started a trial.
   */
