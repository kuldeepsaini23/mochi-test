 ...
 *
 * WHY: Provides UI for users to create a new account
 * HOW: Uses Better Auth client directly for authentication
 *
 * INVITATION FLOW:
 * - When invitationId is present, email comes from URL params (locked)
 * - accept-invitation page passes: invitationId, email, org
 * - Email field is read-only to prevent signing up with wrong email
 *
 * PERFORMANCE: After successful sign-up, redirects directly to dashboard.
...
export function SignUpForm() {
  ...
  const trackReferral = trpc.affiliate.trackReferral.useMutation()

  /**
   * Portal Admin Check Mutation
   * WHY: After successful sign-up, check if user is a portal admin
  ...
  // Check for affiliate cookie on mount
  useEffect(() => {
  ...
  async function onSubmit(values: SignupFormValues) {
    ...
    if (isEmailLocked && values.email.toLowerCase() !== invitedEmail?.toLowerCase()) {
      setError('You can only sign up with the email address the invitation was sent to.')
      setLoading(false)
      return
    }
    ...
    try {
      ...
      if (affiliateCode) {
        try {
          await trackReferral.mutateAsync({ affiliateCode })
       ...
       * WHY: Better Auth's signUp.email() returns success with a Set-Cookie header,
      ...
      // This avoids 2-4+ database queries on the sign-up page
    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col items-center gap-2 text-center">
        ...
        <p className="text-muted-foreground text-sm text-balance">
          {isEmailLocked
            ? `Create your account to join ${organizationName || 'the organization'}`
      ...
      <div className="text-center text-sm">
        ...
        <Link
          href={
            invitationId
              ? `/sign-in?invitationId=${invitationId}${invitedEmail ? `&email=${encodeURIComponent(invitedEmail)}` : ''}${organizationName ? `&org=${encodeURIComponent(organizationName)}` : ''}`
              : '/sign-in'
          }
